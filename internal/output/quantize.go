...
func quantizeOrdered(rows, indexed [][]byte, info *ImageInfo, cm *Colormap) {
	levels := orderedDitherLevels(cm)
	quantizeNearest(rows, indexed, info, cm, func(x, y int, p []int) {
		for i := range p {
			p[i] = clamp8(p[i] + orderedDitherAdjust(levels[i], x, y))
		}
	})
}

func orderedDitherLevels(cm *Colormap) []int {
	if cm == nil || len(cm.Maps) == 0 {
		return []int{2, 2, 2}
	}
	levels := make([]int, len(cm.Maps))
	for i, m := range cm.Maps {
		seen := make(map[byte]struct{}, cm.NumColors)
		for j := 0; j < cm.NumColors && j < len(m); j++ {
			seen[m[j]] = struct{}{}
		}
		levels[i] = len(seen)
		if levels[i] < 2 {
...
func orderedDitherAdjust(levels, x, y int) int {
	if levels <= 1 {
		return 0
	}
	const cells = 16 * 16
	fill := ijgBaseDitherMatrix[y&15][x&15]
	num := (cells - 1 - 2*fill) * 255
	den := 2 * cells * (levels - 1)
	return num / den
}

var ijgBaseDitherMatrix = [16][16]int{
