...
func (d *Decoder) quantizeRaster(src *Raster) (*Raster, error) {
	...
	out.Palette = paletteFromOutputColormap(qmap, src.Format)
	return out, nil
}
...
func outputColorSpaceForRaster(format PixelFormat) (internaloutput.ColorSpace, error) {
	...
	case PixelFormatBigGamutYCbCr24:
		return internaloutput.ColorSpaceBigGamutYCbCr, nil
	case PixelFormatCMYK32:
	...
	default:
		return internaloutput.ColorSpaceRGB,
			fmt.Errorf("%w: quantized output supports gray, rgb, ycbcr, big-gamut-ycbcr, cmyk, and ycck output, got %s", ErrUnsupported, format)
	}
}
...
func outputColormapFromPalette(palette color.Palette, format PixelFormat) (*internaloutput.Colormap, error) {
	...
	case PixelFormatBigGamutYCbCr24:
		...
		for i, c := range palette {
			r, g, b, _ := c.RGBA()
			luma[i], cbMap[i], crMap[i] = rgbToBigGamutYCbCr(byte(r>>8), byte(g>>8), byte(b>>8))
		}
		return &internaloutput.Colormap{Maps: [][]uint8{luma, cbMap, crMap}, NumColors: len(palette)}, nil
	case PixelFormatCMYK32:
	...
	case PixelFormatYCCK32:
		...
		for i, c := range palette {
			cmyk := color.CMYKModel.Convert(c).(color.CMYK)
			luma[i], cbMap[i], crMap[i] = color.RGBToYCbCr(255-cmyk.C, 255-cmyk.M, 255-cmyk.Y)
			black[i] = cmyk.K
		}
		return &internaloutput.Colormap{Maps: [][]uint8{luma, cbMap, crMap, black}, NumColors: len(palette)}, nil
	default:
		return nil, fmt.Errorf("%w: quantized output supports gray, rgb, ycbcr, big-gamut-ycbcr, cmyk, and ycck output, got %s", ErrUnsupported, format)
	}
}

func paletteFromOutputColormap(cmap *internaloutput.Colormap, format PixelFormat) color.Palette {
	...
	if len(cmap.Maps) >= 4 {
		...
		black := cmap.Maps[3]
		for i := range palette {
			c, m, y := first[i], second[i], third[i]
			if format == PixelFormatYCCK32 {
				c, m, y = yccToCMY(first[i], second[i], third[i])
			}
			palette[i] = color.CMYK{C: c, M: m, Y: y, K: black[i]}
		}
		return palette
	}
	red := cmap.Maps[0]
	...
	for i := range palette {
		...
		} else if format == PixelFormatBigGamutYCbCr24 {
			r, g, b = bigGamutYCbCrToRGB(red[i], green[i], blue[i])
		}
...
func (d *Decoder) applyQuantizationToConfig(cfg *Config) error {
	...
	case PixelFormatGray8, PixelFormatRGB24, PixelFormatYCbCr24, PixelFormatBigGamutYCbCr24, PixelFormatCMYK32, PixelFormatYCCK32:
	default:
		return fmt.Errorf("%w: quantized output supports gray, rgb, ycbcr, big-gamut-ycbcr, cmyk, and ycck output, got %s", ErrUnsupported, cfg.PixelFormat)
	}
	cfg.Components = 1
