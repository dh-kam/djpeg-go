...
func TestDecodeRasterYCbCrQuantizedOutput(t *testing.T) {
	data, err := os.ReadFile("tests/testdata/color_8x8_444.jpg")
	if err != nil {
		t.Skipf("fixture missing: %v", err)
	}
	...
	cfg, err := djpeg.DecodeRasterConfig(
		bytes.NewReader(data),
		djpeg.WithOutputColorSpace(djpeg.ColorSpaceYCbCr),
		djpeg.WithQuantizeColors(8),
		djpeg.WithDitherMode(djpeg.DitherNone),
	)
	if err != nil {
		t.Fatalf("DecodeRasterConfig YCbCr quantized failed: %v", err)
	}
	if !cfg.Quantized || cfg.PixelFormat != djpeg.PixelFormatIndexed8 || cfg.ColorSpace != djpeg.ColorSpaceYCbCr {
		t.Fatalf("YCbCr quantized config = %+v, want indexed8 ycbcr quantized output", cfg)
	}
	...
	if err != nil {
		t.Fatalf("DecodeRaster YCbCr quantized failed: %v", err)
	}
	if raster.Format != djpeg.PixelFormatIndexed8 || len(raster.Palette) == 0 || len(raster.Palette) > 8 {
		t.Fatalf("YCbCr quantized raster format=%s palette=%d, want indexed8 with 1..8 colors",
			raster.Format, len(raster.Palette))
	}
	if _, ok := raster.Palette[0].(color.RGBA); !ok {
		t.Fatalf("YCbCr quantized palette entry type = %T, want color.RGBA", raster.Palette[0])
	}
	for i, idx := range raster.Pix {
		if int(idx) >= len(raster.Palette) {
			t.Fatalf("pixel %d index=%d outside palette length %d", i, idx, len(raster.Palette))
